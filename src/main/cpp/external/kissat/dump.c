...
{
  const queue *const queue = &solver->queue_;
  ...
  const links *const links = solver->links_;
...
dump_export (kissat * solver)
{
  ...
  for (unsigned idx = 0; idx < size; idx++)
    printf ("export[%u] = %u\n", LIT (idx), PEEK_STACK (solver->export_, idx));
}
...
dump_import (kissat * solver)
{
  ...
  for (unsigned idx = 1; idx < size; idx++)
      ...
      else if (import->eliminated)
	{
	  ...
	  if (pos < SIZE_STACK (solver->eliminated_))
	    {
	      int value = PEEK_STACK (solver->eliminated_, pos);
	      if (value)
