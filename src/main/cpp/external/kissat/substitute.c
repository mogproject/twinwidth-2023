...
determine_representatives (kissat * solver, unsigned *repr)
{
  ...
  watches *all_watches = solver->watches_;
  ...
  for (all_stack (litwatch, litwatch, delayed_deleted))
    {
      ...
      const watch watch = litwatch.watch_;
...
substitute_clauses (kissat * solver, unsigned *repr)
{
  ...
  for (all_clauses (c))
      ...
      for (all_literals_in_clause (lit, c))
	{
	  ...
	  marks[repr_lit] = true;
	  PUSH_STACK (solver->clause_, repr_lit);
	}
	...
	{
	  const unsigned size = SIZE_STACK (solver->clause_);
	  if (!size)
	    {
	  ...
	  else
	    {
	      ...
	      const unsigned new_size = SIZE_STACK (solver->clause_);
	      unsigned *new_lits = BEGIN_STACK (solver->clause_);
...
substitute_rounds (kissat * solver)
{
  ...
  for (unsigned round = 1; round <= maxrounds; round++)
	...
	break;
      ...
      const uint64_t reference = solver->statistics_.search_ticks -
	solver->last.probe;
