...
do { \
  solver->phases_.NAME = \
    kissat_realloc (solver, solver->phases_.NAME, old_size, new_size); \
} while (0)
...
do { \
  ...
  realloc_phases (NAME); \
  memset (solver->phases_.NAME + old_size, 0, new_size - old_size); \
} while (0)
...
#define release_phases(NAME, SIZE) \
  kissat_free (solver, solver->phases_.NAME, SIZE)
...
kissat_save_best_phases (kissat * solver)
{
  ...
  save_phases (solver, solver->phases_.best);
}
...
kissat_save_saved_phases (kissat * solver)
{
  ...
  save_phases (solver, solver->phases_.saved);
}
...
kissat_save_target_phases (kissat * solver)
{
  ...
  save_phases (solver, solver->phases_.target);
}
