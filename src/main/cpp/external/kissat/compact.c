...
compact_literal (kissat * solver, unsigned dst_lit, unsigned src_lit)
{
  ...
  solver->assigned_[dst_idx] = solver->assigned_[src_idx];
  solver->flags_[dst_idx] = solver->flags_[src_idx];

  solver->phases_.best[dst_idx] = solver->phases_.best[src_idx];
  solver->phases_.saved[dst_idx] = solver->phases_.saved[src_idx];
  solver->phases_.target[dst_idx] = solver->phases_.target[src_idx];
...
compact_queue (kissat * solver)
{
  ...
  links *links = solver->links_, *l;
  unsigned *p = &solver->queue_.first, prev = DISCONNECT;
  solver->queue_.stamp = 0;
  for (unsigned idx; !DISCONNECTED (idx = *p); p = &l->next)
    {
      ...
      l = links + idx;
      l->prev = prev;
      l->stamp = ++solver->queue_.stamp;
      if (idx == solver->queue_.search.idx)
	{
	  solver->queue_.search.idx = midx;
	  solver->queue_.search.stamp = l->stamp;
	}
      *p = prev = midx;
    }
  solver->queue_.last = prev;
  *p = DISCONNECT;
  ...
  for (size_t i = 0; i < size; i++)
    {
      ...
      assigned *a = solver->assigned_ + idx;
      if (!a->binary)
	continue;
...
compact_frames (kissat * solver)
{
  ...
  const size_t size = SIZE_STACK (solver->frames_);
...
compact_best_and_target_values (kissat * solver, unsigned vars)
{
  ...
  const value *const target = solver->phases_.target;
  const flags *const flags = solver->flags_;
...
kissat_finalize_compacting (kissat * solver, unsigned vars, unsigned mfixed)
{
  ...
  memset (solver->assigned_ + vars, 0, reduced * sizeof (assigned));
  memset (solver->flags_ + vars, 0, reduced * sizeof (flags));
  memset (solver->values + 2 * vars, 0, 2 * reduced * sizeof (value));
  memset (solver->watches_ + 2 * vars, 0, 2 * reduced * sizeof (watches));
