...
report_resized (kissat * solver, const char *mode, arena before)
{
  ...
  ward *const new_begin = BEGIN_STACK (solver->arena_);
  const bool moved = (new_begin != old_begin);
  const uint64_t capacity = CAPACITY_STACK (solver->arena_);
...
kissat_allocate_clause (kissat * solver, size_t size)
{
  ...
  const size_t res = SIZE_STACK (solver->arena_);
  assert (res <= MAX_REF);
  ...
  if (needed > available)
      ...
      do
	{
	  ...
	  kissat_stack_enlarge (solver, (chars *) & solver->arena_,
				sizeof (ward));
	  capacity = CAPACITY_STACK (solver->arena_);
...
kissat_clause_in_arena (const kissat * solver, const clause * c)
{
  if (!kissat_aligned_pointer (c))
    return false;
  const char *p = (char *) c;
  const char *begin = (char *) BEGIN_STACK (solver->arena_);
  const char *end = (char *) END_STACK (solver->arena_);
  if (p < begin)
    return false;
