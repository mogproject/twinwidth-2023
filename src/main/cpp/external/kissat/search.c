...
{
  if (solver->limited_.conflicts)
    {
      LOG ("reset conflict limit");
      solver->limited_.conflicts = false;
    }

  if (solver->limited_.decisions)
    {
      LOG ("reset decision limit");
      solver->limited_.decisions = false;
    }

  if (solver->termination_.flagged)
    {
      kissat_very_verbose (solver, "termination forced externally");
      solver->termination_.flagged = 0;
    }
...
conflict_limit_hit (kissat * solver)
{
  if (!solver->limited_.conflicts)
    return false;
  if (solver->limits_.conflicts > solver->statistics_.conflicts)
    return false;
  kissat_very_verbose (solver, "conflict limit %" PRIu64
		       " hit after %" PRIu64 " conflicts",
		       solver->limits_.conflicts,
...
decision_limit_hit (kissat * solver)
{
  if (!solver->limited_.decisions)
    return false;
  if (solver->limits_.decisions > solver->statistics_.decisions)
    return false;
  kissat_very_verbose (solver, "decision limit %" PRIu64
		       " hit after %" PRIu64 " decisions",
		       solver->limits_.decisions,
...
kissat_search (kissat * solver)
{
  ...
  while (!res)
    {
      if (TERMINATED (search_terminated_1)) break;
