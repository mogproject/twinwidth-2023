...
{
  if (!solver->enabled_.eliminate)
    return false;
  statistics *statistics = &solver->statistics_;
  if (!statistics->clauses_irredundant)
    return false;
  if (solver->waiting_.eliminate.reduce > statistics->reductions)
    return false;
  ...
  for (all_stack (unsigned, other, solver->resolvents))
      ...
      else if (!satisfied)
	{
	  ...
	  if (value > 0)
	    ...
	    PUSH_STACK (solver->clause_, other);
	}
    }
...
try_to_eliminate_all_variables_again (kissat * solver)
{
  ...
  flags *all_flags = solver->flags_;
  for (all_variables (idx))
    {
      flags *flags = all_flags + idx;
      flags->eliminate = true;
    }
  solver->limits_.eliminate.variables.removed = 0;
}
...
set_next_elimination_bound (kissat * solver, bool complete)
{
  ...
  const unsigned current_bound = solver->bounds_.eliminate.additional_clauses;
  ...
  if (complete)
...
#ifndef QUIET
	  bool first = !solver->bounds_.eliminate.max_bound_completed++;
	  REPORT (!first, first ? '!' : ':');
#endif
	}
      else
	{
	  ...
	  solver->bounds_.eliminate.additional_clauses = next_bound;
...
eliminate_variables (kissat * solver)
{
  kissat_very_verbose (solver,
		       ...
		       solver->bounds_.eliminate.additional_clauses);
  assert (!solver->inconsistent);
#ifndef QUIET
	  ...
	  if (solver->statistics_.eliminate_resolutions > resolution_limit)
	    {
	      kissat_extremely_verbose (solver,
					...
					solver->
					statistics_.eliminate_resolutions);
	      complete = false;
	      break;
	    }
	  tried++;
...
eliminate (kissat * solver)
{
  ...
  kissat_phase (solver, "eliminate", GET (eliminations),
		...
		solver->limits_.eliminate.conflicts);
...
kissat_eliminate (kissat * solver)
{
  ...
  solver->waiting_.eliminate.reduce = solver->statistics_.reductions + 1;
  solver->last.eliminate = solver->statistics_.search_ticks;
