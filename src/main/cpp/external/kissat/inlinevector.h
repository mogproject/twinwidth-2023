...
kissat_begin_vector (kissat * solver, vector * vector)
{
#ifdef COMPACT
  return BEGIN_STACK (solver->vectors_.stack) + vector->offset;
#else
  (void) solver;
  return vector->begin;
#endif
...
static inline unsigned *
kissat_end_vector (kissat * solver, vector * vector)
{
#ifdef COMPACT
...
kissat_offset_vector (kissat * solver, vector * vector)
{
#ifdef COMPACT
  (void) solver;
  ...
  unsigned *begin_stack = BEGIN_STACK (solver->vectors_.stack);
...
static inline size_t
kissat_size_vector (const vector * vector)
{
#ifdef COMPACT
  return vector->size;
#else
...
static inline bool
kissat_empty_vector (vector * vector)
{
#ifdef COMPACT
  return !vector->size;
#else
...
static inline void
kissat_inc_usable (kissat * solver)
{
  assert (MAX_SECTOR > solver->vectors_.usable);
  solver->vectors_.usable++;
}

static inline void
kissat_add_usable (kissat * solver, size_t inc)
{
  assert (MAX_SECTOR - inc >= solver->vectors_.usable);
  solver->vectors_.usable += inc;
}

static inline unsigned *
kissat_last_vector_pointer (kissat * solver, vector * vector)
{
...
static inline void
kissat_pop_vector (kissat * solver, vector * vector)
{
  assert (!kissat_empty_vector (vector));
#ifdef COMPACT
...
static inline void
kissat_release_vector (kissat * solver, vector * vector)
{
  kissat_resize_vector (solver, vector, 0);
}

static inline void
kissat_dec_usable (kissat * solver)
{
  assert (solver->vectors_.usable > 0);
  solver->vectors_.usable--;
}

static inline void
kissat_push_vectors (kissat * solver, vector * vector, unsigned e)
{
  unsigneds *stack = &solver->vectors_.stack;
