...
kissat_deduce_first_uip_clause (kissat * solver, clause * conflict)
{
  ...
  PUSH_STACK (solver->clause_, INVALID_LIT);
  ...
  assert (PEEK_STACK (solver->clause_, 0) == INVALID_LIT);
  POKE_STACK (solver->clause_, 0, NOT (uip));
  LOGTMP ("deduced not yet minimized 1st UIP");
  if (!solver->probing)
    ADD (literals_deduced, SIZE_STACK (solver->clause_));
  STOP (deduce);
  return 0;
}
