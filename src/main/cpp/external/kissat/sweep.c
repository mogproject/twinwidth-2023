...

  {
	...
	if (head.type.binary)
	...
	else
	  {
		...
		if (other == lit)
		  {
		    ...
		    found = true;
#endif
		    PUSH_STACK (solver->clause_, repr);
		    continue;
		  }
		...
		const unsigned unit = POP_STACK (solver->clause_);
		CHECK_AND_ADD_UNIT (unit);
		ADD_UNIT_TO_PROOF (unit);
	    ...
	    if (!c->redundant)
	      kissat_mark_added_literals (solver, new_size,
					  BEGIN_STACK (solver->clause_));

	    if (new_size == 2)
	      {
		const unsigned second = POP_STACK (solver->clause_);
		const unsigned first = POP_STACK (solver->clause_);
	    ...
	    const unsigned *const begin = BEGIN_STACK (solver->clause_);
	    const unsigned *const end = END_STACK (solver->clause_);
