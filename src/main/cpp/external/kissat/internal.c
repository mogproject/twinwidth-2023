...
#ifndef NOPTIONS
  kissat_init_options (&solver->options_);
#else
  kissat_init_options ();
#endif
#ifndef QUIET
  kissat_init_profiles (&solver->profiles_);
#endif
  START (total);
...
kissat_release (kissat * solver)
{
  ...
  DEALLOC_VARIABLE_INDEXED (assigned_);
  DEALLOC_VARIABLE_INDEXED (flags_);
  DEALLOC_VARIABLE_INDEXED (links_);

  DEALLOC_LITERAL_INDEXED (marks);
  DEALLOC_LITERAL_INDEXED (values);
  DEALLOC_LITERAL_INDEXED (watches_);

  RELEASE_STACK (solver->import);
  RELEASE_STACK (solver->eliminated_);
  RELEASE_STACK (solver->extend);
  RELEASE_STACK (solver->witness);
  RELEASE_STACK (solver->etrail);

  RELEASE_STACK (solver->vectors_.stack);
  RELEASE_STACK (solver->delayed);

  RELEASE_STACK (solver->clause_);
  RELEASE_STACK (solver->shadow);
#if defined(LOGGING) || !defined(NDEBUG)
  RELEASE_STACK (solver->resolvent);
#endif

  RELEASE_STACK (solver->arena_);

  RELEASE_STACK (solver->units);
  RELEASE_STACK (solver->frames_);
...
#ifndef QUIET
  RELEASE_STACK (solver->profiles_.stack);
#endif
...
#ifndef NOPTIONS
  return kissat_options_get (&solver->options_, name);
#else
  (void) solver;
...
kissat_set_option (kissat * solver, const char *name, int new_value)
{
  ...
  kissat_require (name, "name zero pointer");
#ifndef NOPTIONS
  return kissat_options_set (&solver->options_, name, new_value);
#else
...
kissat_set_decision_limit (kissat * solver, unsigned limit)
{
  ...
  limited *limited = &solver->limited_;
...
kissat_add (kissat * solver, int elit)
{
  ...
  if (elit)
      ...
      if (!mark)
	{
	  ...
	  if (value > 0)
	  ...
	  else
	    {
	      MARK (ilit) = 1;
	      MARK (NOT (ilit)) = -1;
	      assert (SIZE_STACK (solver->clause_) < UINT_MAX);
	      PUSH_STACK (solver->clause_, ilit);
	    }
	}
      else if (mark < 0)
	{
  ...
  else
    {
      ...
      const size_t isize = SIZE_STACK (solver->clause_);
      unsigned *ilits = BEGIN_STACK (solver->clause_);
      ...
      else
	{
	  ...
	  if (!isize)
	    ...
	    {
	      unsigned unit = TOP_STACK (solver->clause_);
...
{
  kissat_require_initialized (solver);
  kissat_require (EMPTY_STACK (solver->clause_),
...
{
  kissat_require_initialized (solver);
  solver->termination_.flagged = ~(unsigned) 0;
  assert (solver->termination_.flagged);
}

void
kissat_set_terminate (kissat * solver, void *state, int (*terminate) (void *))
{
  solver->termination_.terminate = 0;
  solver->termination_.state = state;
  solver->termination_.terminate = terminate;
}
...
kissat_value (kissat * solver, int elit)
{
  ...
  if (import->eliminated)
	...
	kissat_extend (solver);
      ...
      tmp = PEEK_STACK (solver->eliminated_, eliminated);
    }
  else
    {
