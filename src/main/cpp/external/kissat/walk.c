...
import_decision_phases (walker * walker)
{
  ...
  const value *const target =
    (solver->stable && !GET_OPTION (warmup)) ? solver->phases_.target : 0;
...
save_all_values (kissat * solver, walker * walker)
{
  ...
  const value *const values = solver->values;
  value *saved = solver->phases_.saved;
  for (all_variables (idx))
    {
      const unsigned lit = LIT (idx);
      const value value = values[lit];
...
check_walk (kissat * solver, unsigned expected)
{
  ...
  for (all_literals (lit))
	...
	continue;
      ...
      if (!value)
	{
	  value = solver->phases_.saved[IDX (lit)];
	  assert (value);
	  if (NEGATED (lit))
	    value = -value;
	}
	...
	if (watch.type.binary)
	  {
	    ...
	    if (!value)
	      {
		value = solver->phases_.saved[IDX (other)];
		assert (value);
		if (NEGATED (other))
		  value = -value;
	      }
...
kissat_walking (kissat * solver)
{
  ...
  if (last_irredundant == INVALID_REF)
    last_irredundant = SIZE_STACK (solver->arena_);
