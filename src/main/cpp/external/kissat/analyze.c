			       ...
			       unsigned *conflict_level_ptr)
{
  ...
  assigned *all_assigned = solver->assigned_;
  ...
  *conflict_level_ptr = conflict_level;
  ...
  kissat_backtrack_after_conflict (solver, solver->level - 1);
  if (conflict_size == 2)
    {
...
static inline void
mark_reason_side_literal (kissat * solver,
			  assigned * all_assigned, unsigned lit)
{
  const unsigned idx = IDX (lit);
...
static inline void
analyze_reason_side_literal (kissat * solver, size_t limit, ward * arena,
			     assigned * all_assigned, unsigned lit)
{
  const unsigned idx = IDX (lit);
  const assigned *a = all_assigned + idx;
  ...
  if (a->binary)
  ...
  else
    {
      ...
      assert (ref < SIZE_STACK (solver->arena_));
...
analyze_reason_side_literals (kissat * solver)
{
  ...
  if (solver->probing)
    return;
  if (solver->delays_.bumpreasons.count)
    {
      solver->delays_.bumpreasons.count--;
  ...
  if (decision_rate >= decision_rate_limit)
    {
      LOG ("decision rate %g >= limit %d",
	   decision_rate, decision_rate_limit);
      return;
    }
  assigned *all_assigned = solver->assigned_;
#ifndef NDEBUG
  for (all_stack (unsigned, lit, solver->clause_))
  ...
  ward *arena = BEGIN_STACK (solver->arena_);
  for (all_stack (unsigned, lit, solver->clause_))
    {
  ...
  if (SIZE_STACK (solver->analyzed) > limit)
      ...
      while (SIZE_STACK (solver->analyzed) > saved)
	{
      ...
      if (solver->delays_.bumpreasons.current < UINT_MAX)
	{
	  solver->delays_.bumpreasons.current++;
	  LOG ("solver delay bump reasons interval increased to %u",
	       solver->delays_.bumpreasons.current);

	}
    }
  else if (solver->delays_.bumpreasons.current)
    {
      solver->delays_.bumpreasons.current /= 2;
  ...
  solver->delays_.bumpreasons.count = solver->delays_.bumpreasons.current;
...
sort_deduced_clause (kissat * solver)
{
  sort_levels (solver);
#ifndef NDEBUG
  const size_t size_frames = SIZE_STACK (solver->frames_);
#endif
  frame *frames = BEGIN_STACK (solver->frames_);
  unsigned pos = 1;
  ...
  unsigneds *clause = &solver->clause_;
  ...
  const assigned *const assigned = solver->assigned_;
...
static void
reset_analysis_but_not_analyzed_literals (kissat * solver)
{
  reset_removable (solver);
  reset_levels (solver);
  LOG ("reset %zu learned literals", SIZE_STACK (solver->clause_));
  CLEAR_STACK (solver->clause_);
}
...
static void
update_decision_rate_average (kissat * solver)
{
  assert (!solver->probing);
  const uint64_t current = DECISIONS;
  const uint64_t previous = solver->averages_[solver->stable].saved_decisions;
  assert (previous <= current);
  const uint64_t decisions = current - previous;
  solver->averages_[solver->stable].saved_decisions = current;
  UPDATE_AVERAGE (decision_rate, decisions);
}

static void
analyze_failed_literal (kissat * solver, clause * conflict)
{
  assert (solver->level == 1);
  const unsigned failed = FRAME (1).decision;

  LOGCLS (conflict, "analyzing failed literal %s conflict", LOGLIT (failed));

  unsigneds *units = &solver->clause_;
