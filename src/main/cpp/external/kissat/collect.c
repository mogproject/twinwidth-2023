				  ...
				  unsigned lit, bool compact, reference start)
{
  ...
  const assigned *const all_assigned = solver->assigned_;
...
#ifndef NDEBUG
  const size_t size = SIZE_STACK (solver->arena_);
  assert ((size_t) ref <= size);
#endif
  clause *begin = (clause *) (BEGIN_STACK (solver->arena_) + ref);
  clause *end = (clause *) END_STACK (solver->arena_);
...
sparse_sweep_garbage_clauses (kissat * solver, bool compact, reference start)
{
  ...
  clause *begin = (clause *) BEGIN_STACK (solver->arena_);
  const clause *const end = (clause *) END_STACK (solver->arena_);
  ...
  size_t bytes = (char *) END_STACK (solver->arena_) - (char *) dst;
#endif
#ifndef QUIET
  if (flushed)
  ...
  SET_END_OF_STACK (solver->arena_, (ward *) dst);
...
rewatch_clauses (kissat * solver, reference start)
{
  ...
  watches *watches = solver->watches_;
  ward *const arena = BEGIN_STACK (solver->arena_);

  clause *end = (clause *) END_STACK (solver->arena_);
  clause *c = (clause *) (BEGIN_STACK (solver->arena_) + start);
  assert (c <= end);
