...
init_enabled (kissat * solver)
{
  ...
  solver->enabled_.eliminate = eliminate;
}
...
{
  assert (solver->statistics_.searches == 1);
  ...
  if (solver->enabled_.eliminate)
    {
      ...
      solver->bounds_.eliminate.max_bound_completed = 0;
      solver->bounds_.eliminate.additional_clauses = 0;
