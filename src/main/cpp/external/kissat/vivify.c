...
simplify_vivification_candidate (kissat * solver, clause * const c)
{
  ...
  assert (EMPTY_STACK (solver->clause_));
  ...
  for (all_literals_in_clause (lit, c))
      ...
      if (value > 0)
	{
      ...
      if (!value)
	PUSH_STACK (solver->clause_, lit);
    }
  unsigned non_false = SIZE_STACK (solver->clause_);
  if (satisfied)
    {
  ...
  if (non_false == 2)
    {
      const unsigned first = PEEK_STACK (solver->clause_, 0);
      const unsigned second = PEEK_STACK (solver->clause_, 1);
    ...
    {
      CHECK_AND_ADD_STACK (solver->clause_);
      ADD_STACK_TO_PROOF (solver->clause_);
				  ...
				  unsigned *const counts, bool tier2)
{
  ...
  ward *const arena = BEGIN_STACK (solver->arena_);
		...
		clause * conflict, bool *irredundant_ptr)
{
  ...
  while (analyzed < SIZE_STACK (solver->analyzed))
      ...
      if (a->reason == DECISION_REASON)
	{
	  ...
	  PUSH_STACK (solver->clause_, not_lit);
	}
      else if (a->binary)
	{
  ...
  const size_t size = SIZE_STACK (solver->clause_);
  assert (size > 0);
  if (subsumed)
    {
      if (size == 1)
	{
	  ...
	  const unsigned unit = PEEK_STACK (solver->clause_, 0);
...
{
  struct assigned *assigned = solver->assigned_;
	      ...
	      unsigned non_false, bool irredundant, unsigned implied)
{
  ...
  size_t size = SIZE_STACK (solver->clause_);
...
kissat_vivify (kissat * solver)
{
  ...
  const uint64_t delta = ticks_limit - solver->statistics_.probing_ticks;
