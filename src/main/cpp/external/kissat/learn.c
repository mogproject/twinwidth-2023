...
{
  assert (not_uip == PEEK_STACK (solver->clause_, 0));
...
{
  const unsigned other = PEEK_STACK (solver->clause_, 1);
...
learn_reference (kissat * solver, unsigned not_uip, unsigned glue)
{
  ...
  assert (SIZE_STACK (solver->clause_) > 2);
  unsigned *lits = BEGIN_STACK (solver->clause_);
...
kissat_update_learned (kissat * solver, unsigned glue, unsigned size)
{
  ...
  if (solver->stable)
    kissat_tick_reluctant (&solver->reluctant_);
...
{
  const unsigned not_uip = PEEK_STACK (solver->clause_, 0);
