...
kissat_reducing (kissat * solver)
{
  ...
  if (!solver->statistics_.clauses_redundant)
    return false;
  if (CONFLICTS < solver->limits_.reduce.conflicts)
    return false;
  return true;
}
...
collect_reducibles (kissat * solver, reducibles * reds, reference start_ref)
{
  ...
  assert (start_ref <= SIZE_STACK (solver->arena_));
