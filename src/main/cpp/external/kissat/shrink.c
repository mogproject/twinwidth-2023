...
{
  assert (solver->assigned_ == assigned);
  assert (VALUE (lit) < 0);
...
push_literals_of_block (kissat * solver, assigned * assigned,
			...
			unsigned level)
{
  assert (assigned == solver->assigned_);
...
kissat_shrink_clause (kissat * solver)
{
  ...
  unsigned *begin_lits = BEGIN_STACK (solver->clause_);
  unsigned *end_lits = END_STACK (solver->clause_);

  unsigned *end_block = END_STACK (solver->clause_);
  ...
  SET_END_OF_STACK (solver->clause_, q);
