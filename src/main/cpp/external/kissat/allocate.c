...
inc_bytes (kissat * solver, size_t bytes)
{
  ...
  LOG5 ("allocated_current = %s",
	FORMAT_BYTES (solver->statistics_.allocated_current));
  if (solver->statistics_.allocated_current >=
      solver->statistics_.allocated_max)
    {
      solver->statistics_.allocated_max = solver->statistics_.allocated_current;
      LOG5 ("allocated_max = %s",
	    FORMAT_BYTES (solver->statistics_.allocated_max));
    }
#else
  (void) solver;
  (void) bytes;
