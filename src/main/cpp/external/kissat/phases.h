...
#define BEST(IDX) \
  (solver->phases_.best[assert (VALID_INTERNAL_INDEX (IDX)), (IDX)])

#define SAVED(IDX) \
  (solver->phases_.saved[assert (VALID_INTERNAL_INDEX (IDX)), (IDX)])

#define TARGET(IDX) \
  (solver->phases_.target[assert (VALID_INTERNAL_INDEX (IDX)), (IDX)])
...
#define all_phases(NAME,PTR) \
  value * PTR = solver->phases_.NAME, * const end_ ## PTR = PTR + VARS; \
  PTR != end_ ## PTR; \
  ++PTR
