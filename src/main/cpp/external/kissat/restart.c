...
kissat_restarting (kissat * solver)
{
  ...
  if (!solver->level)
    return false;
  if (CONFLICTS < solver->limits_.restart.conflicts)
    return false;
  if (solver->stable)
    return kissat_reluctant_triggered (&solver->reluctant_);
...
void
kissat_update_focused_restart_limit (kissat * solver)
{
...
kissat_restart (kissat * solver)
{
  ...
  kissat_extremely_verbose (solver,
			    ...
			    solver->limits_.restart.conflicts);
