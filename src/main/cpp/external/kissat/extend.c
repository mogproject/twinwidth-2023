...
static void
undo_eliminated_assignment (kissat * solver)
{
  size_t size_etrail = SIZE_STACK (solver->etrail);
#ifdef LOGGING
  size_t size_eliminated = SIZE_STACK (solver->eliminated_);
#endif
  if (!size_etrail)
    {
  ...
  value *values = BEGIN_STACK (solver->eliminated_);

  while (!EMPTY_STACK (solver->etrail))
    {
      const unsigned pos = POP_STACK (solver->etrail);
      assert (pos < SIZE_STACK (solver->eliminated_));
      assert (values[pos]);
...
static void
extend_assign (kissat * solver, value * values, int lit)
{
  assert (lit);
  assert (lit != INT_MIN);
...
kissat_extend (kissat * solver)
{
  ...
  value *evalues = BEGIN_STACK (solver->eliminated_);
  ...
  while (p != begin)
	  ...
	  if (import->eliminated)
	    {
	      ...
	      assert (tmp < SIZE_STACK (solver->eliminated_));
		...
		value = -value;
	      ...
	      if (value > 0)
		{
		  LOG2 ("internal literal %s satisfies clause",
			LOGLIT (ilit));
		  satisfied = true;
		}
	    }
      ...
      if (eliminated && eliminated != blocking)
	{
      ...
      assert (blocking_pos < SIZE_STACK (solver->eliminated_));
...
#ifdef LOGGING
  size_t total = SIZE_STACK (solver->eliminated_);
