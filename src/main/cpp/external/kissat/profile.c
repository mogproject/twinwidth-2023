...
{
  profile->entered = now;
  PUSH_STACK (solver->profiles_.stack, profile);
}

void
kissat_profiles_print (kissat * solver)
{
  profiles *named = &solver->profiles_;
...
{
  assert (TOP_STACK (solver->profiles_.stack) == profile);
  (void) POP_STACK (solver->profiles_.stack);
  const double now = kissat_process_time ();
...
kissat_stop_search_and_start_simplifier (kissat * solver, profile * profile)
{
  ...
  const double now = kissat_process_time ();
  while (TOP_STACK (solver->profiles_.stack) != search)
    {
      struct profile *mode = POP_STACK (solver->profiles_.stack);
...
{
  struct profile *simplify = &PROFILE (simplify);
  struct profile *top = POP_STACK (solver->profiles_.stack);
  const double now = kissat_process_time ();
  const double delta = flush_profile (simplify, now);
#ifndef NDEBUG
  const double entered = now - delta;
  assert (solver->mode_.entered <= entered);
#endif
  solver->mode_.entered += delta;
  if (top == profile)
    {
      flush_profile (profile, now);
      assert (TOP_STACK (solver->profiles_.stack) == simplify);
      (void) POP_STACK (solver->profiles_.stack);
    }
  else
    {
...
{
  const double now = kissat_process_time ();
  flush_profiles (&solver->profiles_, now);
